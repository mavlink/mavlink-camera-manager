...
fn build_web() {
    ...
    // The TypeScript bindings under frontend/bindings/ are generated by the
    // `bindings` crate (`cargo run --package=bindings`) and committed to git.
    // CI regenerates them and runs `git diff` to catch unintended API changes
    // (see .githooks/pre-push and .github/workflows/rust.yml).
    // They must exist before the frontend can be built.
