...
impl From<CliCodec> for stream_clients::Codec {
    fn from(c: CliCodec) -> Self {
        match c {
            CliCodec::H264 => stream_clients::Codec::H264,
            CliCodec::H265 => stream_clients::Codec::H265,
        }
    }
}
...
async fn main() -> Result<()> {
    ...
    let args = Args::parse();
    let codec: stream_clients::Codec = args.codec.into();
    ...
    let mut _clients: Vec<Box<dyn std::any::Any>> = Vec::new();
    ...
    for (i, ws_url) in args.webrtc_urls.iter().enumerate() {
        ...
        let (tx, rx) = mpsc::unbounded_channel();
        let client = stream_clients::webrtc_client::WebrtcClient::connect(
            ws_url,
            args.producer_id,
            Some(tx),
            None,
