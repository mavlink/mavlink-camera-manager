    ...
    async fn handle_device(mcontext: Arc<RwLock<ManagerContext>>, device: OnvifDevice) {
        ...
        for url in &device.urls {
            ...
            for stream_information in streams_informations {
                // Embed credentials to the stream uri so we use the credentials to access the RTSP stream
                let mut stream_uri = stream_information.stream_uri.clone();
                if let Some(credentials) = &credentials {
                    let _ = stream_uri.set_username(&credentials.username);
                    let _ = stream_uri.set_password(Some(&credentials.password));
                }

                context
                    .cameras
                    .entry(stream_uri.to_string())
