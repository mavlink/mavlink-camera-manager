...
impl LifecycleState {
    ...
    /// CAS loop: Waking(N>0)→Running(N), Waking(0)→Draining(0). Returns true
    /// only if the transition reached Running. Returns false if the phase was
    /// not Waking (no-op) or if Waking(0) moved to Draining instead.
    pub fn transition_from_waking(&self) -> bool {
        loop {
            let cur = self.packed.load(Ordering::Acquire);
            let (phase, count) = unpack(cur);
            match phase {
    ...
    pub fn assert_invariants(&self) {
        ...
        assert_ne!(
            ...
            "I4 violated: status returned Stopped"
        );

        assert!(
            count <= MAX_COUNT,
            "I5 violated: count={count} exceeds MAX_COUNT"
        );
    }
...
mod tests {
    ...
    #[test]
    fn test_default_constructor_idle_zero() {
    ...
    #[test]
    fn test_add_from_idle_to_waking_notify_fired() {
        let ls = LifecycleState::new();
    ...
    #[test]
    fn test_add_from_waking_no_notify() {
    ...
    #[test]
    fn test_add_from_running_increments() {
    ...
    #[test]
    fn test_add_from_draining_to_running() {
    ...
    #[test]
    fn test_remove_running_decrement_lazy() {
    ...
    #[test]
    fn test_remove_last_running_to_draining_lazy() {
        let ls = LifecycleState::from_raw(pack(Phase::Running, 1));
        let new_count = ls.remove_consumer(true);
    ...
    #[test]
    fn test_remove_nonlazy_keeps_running() {
        let ls = LifecycleState::from_raw(pack(Phase::Running, 1));
    ...
    #[test]
    fn test_remove_last_waking_stays_waking_lazy() {
        let ls = LifecycleState::from_raw(pack(Phase::Waking, 1));
        let new_count = ls.remove_consumer(true);
        assert_eq!(new_count, 0);
        let (phase, count) = ls.load();
    ...
    #[test]
    fn test_transition_from_waking_waking_with_consumers() {
        let ls = LifecycleState::from_raw(pack(Phase::Waking, 3));
        assert!(ls.transition_from_waking());
        let (phase, count) = ls.load();
        assert_eq!(phase, Phase::Running);
        assert_eq!(count, 3);
        ls.assert_invariants();
    ...
    #[test]
    fn test_transition_from_waking_waking_zero_goes_draining() {
    ...
    #[test]
    fn test_transition_from_waking_from_running_noop() {
    ...
    #[test]
    fn test_transition_from_waking_from_draining_noop() {
        let ls = LifecycleState::from_raw(pack(Phase::Draining, 0));
        assert!(!ls.transition_from_waking());
        let (phase, count) = ls.load();
        assert_eq!(phase, Phase::Draining);
    ...
    #[test]
    fn test_transition_from_waking_from_idle_noop() {
        let ls = LifecycleState::new();
        assert!(!ls.transition_from_waking());
        let (phase, count) = ls.load();
        assert_eq!(phase, Phase::Idle);
    ...
    #[test]
    fn test_transition_to_idle_from_draining() {
        let ls = LifecycleState::from_raw(pack(Phase::Draining, 0));
    ...
    #[test]
    fn test_transition_to_idle_fails_if_consumer_reconnected() {
        let ls = LifecycleState::from_raw(pack(Phase::Draining, 0));
        let notify = MockNotify::new();
    ...
    #[test]
    fn test_transition_to_idle_from_running_noop() {
        let ls = LifecycleState::from_raw(pack(Phase::Running, 3));
        assert!(!ls.transition_to_idle());
        let (phase, count) = ls.load();
        assert_eq!(phase, Phase::Running);
    ...
    #[test]
    fn test_pipeline_error_draining_to_idle() {
        let ls = LifecycleState::from_raw(pack(Phase::Draining, 0));
    ...
    #[test]
    fn test_pipeline_error_running_to_waking() {
        let ls = LifecycleState::from_raw(pack(Phase::Running, 3));
        let backoff = ls.handle_pipeline_error();
        assert!(backoff > Duration::ZERO);
        let (phase, count) = ls.load();
    ...
    #[test]
    fn test_pipeline_error_waking_selfloop() {
    ...
    #[test]
    fn test_pipeline_error_idle_noop() {
        let ls = LifecycleState::new();
        let backoff = ls.handle_pipeline_error();
    ...
    #[test]
    fn test_status_draining_is_idle() {
        let ls = LifecycleState::from_raw(pack(Phase::Draining, 0));
        assert_eq!(ls.stream_status(), StreamStatusState::Idle);
    }

    #[test]
    fn test_status_waking_is_running() {
        let ls = LifecycleState::from_raw(pack(Phase::Waking, 1));
    ...
    #[test]
    fn test_status_never_stopped_exhaustive() {
    ...
    #[test]
    fn test_stress_concurrent_add_from_idle() {
        let ls = LifecycleState::new();
        let notify = MockNotify::new();
    ...
    #[test]
    fn test_stress_concurrent_add_remove_churn() {
    ...
    #[test]
    fn test_stress_add_races_transition_to_idle() {
        for _ in 0..1_000 {
            let ls = LifecycleState::from_raw(pack(Phase::Draining, 0));
            let notify = MockNotify::new();
    ...
    #[test]
    fn test_stress_status_never_stopped_under_load() {
        let ls = LifecycleState::new();
        let notify = MockNotify::new();
    ...
    #[test]
    fn test_stress_rapid_lifecycle_with_readers() {
        let ls = LifecycleState::new();
        let notify = MockNotify::new();
    ...
    #[test]
    fn test_stress_pipeline_error_during_add() {
    ...
    #[test]
    fn test_stress_draining_zero_never_persists() {
        for _ in 0..1_000 {
            let ls = LifecycleState::from_raw(pack(Phase::Running, 1));
            let notify = MockNotify::new();
            let barrier = std::sync::Barrier::new(5);
    ...
    proptest! {
        ...
        #[test]
        fn test_prop_random_op_sequence(ops in prop::collection::vec(op_strategy(), 1..200)) {
            let ls = LifecycleState::new();
            let notify = MockNotify::new();
            for op in &ops {
        ...
        #[test]
        fn test_prop_status_consistency(ops in prop::collection::vec(op_strategy(), 1..200)) {
            let ls = LifecycleState::new();
            let notify = MockNotify::new();
            for op in &ops {
                apply_op(&ls, &notify, op);
        ...
        #[test]
        fn test_prop_random_concurrent_ops(
