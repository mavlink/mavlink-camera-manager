...
fn sdp_offer_text(proto: Protocol) -> Option<String> {
    match proto.message {
        Message::Negotiation(Negotiation::MediaNegotiation(media)) => match media.sdp {
            RTCSessionDescription::Offer(sdp) => Some(sdp.sdp),
            _ => None,
        },
        _ => None,
    }
...
async fn test_redirect_webrtc_session_and_sdp_offer() {
    ...
    let sdp_offer = tokio::time::timeout(Duration::from_secs(10), async {
        while let Some(Ok(msg)) = ws_stream.next().await {
            ...
            if let Some(sdp) = sdp_offer_text(proto) {
                return Some(sdp);
            }
        }
        None
