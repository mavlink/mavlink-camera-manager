...
async fn main() {
    ...
    let test_cases = [
        (VideoEncodeType::H264, format!("udp://{address}:5600"), 115.),
        (VideoEncodeType::Mjpg, format!("udp://{address}:5600"), 115.),
        (VideoEncodeType::Yuyv, format!("udp://{address}:5600"), 115.),
        (
    ...
    for (encode, endpoint, expected_pipeline_latency) in test_cases {
        ...
        assert!(
            latencies.stats.latency_mean - baseline_latency
                <= expected_pipeline_latency + baseline_max_jitter
        );
